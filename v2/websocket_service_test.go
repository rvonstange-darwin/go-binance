...
func (s *websocketServiceTestSuite) TestPartialDepthServe() {
	...
	doneC, stopC, _, err := WsPartialDepthServe("ETHBTC", "5", func(event *WsPartialDepthEvent) {
...
func (s *websocketServiceTestSuite) TestPartialDepthServe100Ms() {
	...
	doneC, stopC, _, err := WsPartialDepthServe100Ms("ETHBTC", "5", func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHBTC",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestCombinedPartialDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedPartialDepthServe(symbolLevels, func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHUSDT",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestDepthServe() {
	...
	doneC, stopC, _, err := WsDepthServe("ETHBTC", func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestDepthServe100Ms() {
	...
	doneC, stopC, _, err := WsDepthServe100Ms("ETHBTC", func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedDepthServe(symbols, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe100Ms() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedDepthServe100Ms(symbols, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
			LastUpdateID:  13544037,
...
func (s *websocketServiceTestSuite) TestWsTradeServe() {
	...
	doneC, stopC, _, err := WsTradeServe("BNBBTC", func(event *WsTradeEvent) {
		e := &WsTradeEvent{
			Event:         "trade",
			Time:          123456789,
			Symbol:        "BNBBTC",
...
func (s *websocketServiceTestSuite) TestBookTickerServe() {
	...
	doneC, stopC, _, err := WsBookTickerServe("BTCUSD_200626", func(event *WsBookTickerEvent) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
