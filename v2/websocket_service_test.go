...
func (s *websocketServiceTestSuite) TestPartialDepthServe() {
	...
	doneC, stopC, _, err := WsPartialDepthServe("ETHBTC", "5", func(event *WsPartialDepthEvent, connectionId int) {
...
func (s *websocketServiceTestSuite) TestPartialDepthServe100Ms() {
	...
	doneC, stopC, _, err := WsPartialDepthServe100Ms("ETHBTC", "5", func(event *WsPartialDepthEvent, connectionId int) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHBTC",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestCombinedPartialDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedPartialDepthServe(symbolLevels, func(event *WsPartialDepthEvent, connectionId int) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHUSDT",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestDepthServe() {
	...
	doneC, stopC, _, err := WsDepthServe("ETHBTC", func(event *WsDepthEvent, connectionId int) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestDepthServe100Ms() {
	...
	doneC, stopC, _, err := WsDepthServe100Ms("ETHBTC", func(event *WsDepthEvent, connectionId int) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedDepthServe(symbols, func(event *WsDepthEvent, connectionId int) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe100Ms() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, _, err := WsCombinedDepthServe100Ms(symbols, func(event *WsDepthEvent, connectionId int) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
			LastUpdateID:  13544037,
...
func (s *websocketServiceTestSuite) TestWsAggTradeServe() {
	...
	doneC, stopC, _, err := WsAggTradeServe("ETHBTC", func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedAggTradeServe() {
	...
	doneC, stopC, _, err := WsCombinedAggTradeServe([]string{"ETHBTC"}, func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsMarketStatServe() {
	...
	doneC, stopC, _, err := WsMarketStatServe("BNBBTC", func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedMarketStatServe() {
	...
	doneC, stopC, _, err := WsCombinedMarketStatServe([]string{"BNBBTC"}, func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsAllMarketsStatServe() {
	...
	doneC, stopC, _, err := WsAllMarketsStatServe(func(event WsAllMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestWsTradeServe() {
	...
	doneC, stopC, _, err := WsTradeServe("BNBBTC", func(event *WsTradeEvent, connectionId int) {
		e := &WsTradeEvent{
			Event:         "trade",
			Time:          123456789,
			Symbol:        "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsAllMiniMarketsStatServe() {
	...
	doneC, stopC, _, err := WsAllMiniMarketsStatServe(func(event WsAllMiniMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestBookTickerServe() {
	...
	doneC, stopC, _, err := WsBookTickerServe("BTCUSD_200626", func(event *WsBookTickerEvent, connectionId int) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
...
func (s *websocketServiceTestSuite) TestAllBookTickerServe() {
	...
	doneC, stopC, _, err := WsAllBookTickerServe(func(event *WsBookTickerEvent, connectionId int) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
			Symbol:       "BTCUSD_200626",
			BestBidPrice: "9548.1",
